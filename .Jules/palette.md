...
## 2024-11-12 - Search Field UX Optimization
**Learning:** Search fields without an explicit `ImeAction.Search` show a generic "Done" or "Enter" key on the soft keyboard, making it unintuitive for users to submit the search. Furthermore, failing to dismiss the keyboard upon submission hides the search results.
**Action:** For search input fields in Jetpack Compose, always set `keyboardOptions = KeyboardOptions(imeAction = ImeAction.Search)` and `keyboardActions = KeyboardActions(onSearch = { focusManager.clearFocus() })` to provide an intuitive return key and properly dismiss the software keyboard.
