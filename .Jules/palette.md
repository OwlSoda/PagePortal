...
**Learning:** Inconsistent empty states (using emojis vs custom messages) fragment the user experience and limit accessibility. Centralizing these into a reusable component with vector icons ensures visual consistency and proper scaling.
**Action:** When encountering disparate empty state implementations, refactor them to use a shared `EmptyState` component that accepts `ImageVector` and `contentDescription` for better accessibility and theme adherence.
