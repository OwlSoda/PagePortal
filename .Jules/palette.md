...
**Learning:** Inconsistent empty states (using emojis vs custom text vs shared components) create a disjointed user experience. Emojis lack semantic meaning for screen readers in this context.
**Action:** Centralize empty state logic into a reusable `EmptyState` component that accepts `ImageVector` and `contentDescription`. Refactor existing screens to use this shared component with appropriate Material Icons.
