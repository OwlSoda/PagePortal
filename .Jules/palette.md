...
**Learning:** Reusable components like `EmptyState` accepting `String` icons encourage brittle, inconsistent UI (e.g., emojis vs. text). Refactoring to `ImageVector` enforces the design system and proper accessibility.
**Action:** When designing or refactoring shared UI components, enforce strong typing for visual elements (e.g., `ImageVector` over `String`) to prevent degradation and ensure consistency.
