...
fun AllBooksPage(books: List<UnifiedBookDisplay>, onBookClick: (String) -> Unit) {
     if (books.isEmpty()) {
        EmptyState(
            ...
            message = "This service has no books available."
        )
        return
    }
...
fun AuthorsPage(books: List<UnifiedBookDisplay>, viewModel: LibraryViewModel) {
    ...
    if (authors.isEmpty()) {
        EmptyState(
            ...
            message = "Authors will appear here once books are synced."
        )
        return
    }
    ...
    val seriesList = remember(books) { books.mapNotNull { it.series }.distinct().sorted() }
     ...
     if (seriesList.isEmpty()) {
        EmptyState(
            ...
            message = "Series will appear here once books are synced."
        )
        return
    }
